...
export type SerializedMCQContainerNode = Spread<
    {
        blockId: string;
    },
    SerializedElementNode
...
export class MCQContainerNode extends ElementNode {
    ...
    static clone(node: MCQContainerNode): MCQContainerNode {
        return new MCQContainerNode(node.__blockId, node.__key);
    }
    ...
    static importJSON(serializedNode: SerializedMCQContainerNode): MCQContainerNode {
        const node = $createMCQContainerNode(serializedNode.blockId);
        return node;
    }
}

export function $createMCQContainerNode(blockId?: string): MCQContainerNode {
    return new MCQContainerNode(blockId);
}
